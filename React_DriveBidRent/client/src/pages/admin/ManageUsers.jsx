      ...
      <>
            ...
            display.map(user => (
              <li key={user._id} className={`${type}-item p-4 border-b border-gray-200 flex justify-between items-center transition-all duration-500 ease-in-out`} data-firstname={user.firstName.toLowerCase()} data-lastname={user.lastName.toLowerCase()} data-email={user.email.toLowerCase()}>
                ...
                <div className="action-buttons flex gap-2.5">
                  <button className="btn btn-blue bg-blue-500 px-4 py-2 rounded cursor-pointer font-bold text-white" onClick={() => openModal(user)}>View Details</button>
                  {isMechanic && isPending && <button className="btn btn-green bg-green-500 px-4 py-2 rounded cursor-pointer font-bold text-white" onClick={() => handleAction('approve', user._id, type)}>Approve</button>}
                  <button className="btn btn-red bg-red-500 px-4 py-2 rounded cursor-pointer font-bold text-white" onClick={() => handleAction(isPending ? 'decline' : type === 'buyer' ? 'deleteBuyer' : 'deleteSeller', user._id, type)}>
        ...
        {filtered.length > 5 && (
          <div className="show-more flex justify-center mt-4">
            <button className="show-more-btn bg-orange-500 text-white border-none px-5 py-2.5 rounded cursor-pointer flex items-center gap-1 hover:bg-orange-600 transition-colors" onClick={() => toggleShowMore(type)}>
