...
export default function ChatRoomBuyer({ chatIdProp }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // switching initial fetch complete
  ...
  // mark messages as read when we open the chat / when messages include unread items
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // notify other components (chat list, navbar) that this chat was read
  ...
  useEffect(() => {
    ...
    // polling for new messages since lastFetchedAt
  ...
  useEffect(() => {
    ...
    // On first mount of the component (only once) always scroll to bottom
    ...
    // Do not auto-scroll if user just sent the message or when switching chats
    if (isSendingRef.current || isSwitchingChatRef.current) return;

    // For subsequent updates, only scroll if user was near the bottom
    if (shouldAutoScroll()) {
      messagesRef.current?.scrollIntoView({ behavior: 'smooth' });
    }
  }, [messages]);
  ...
  const handleSend = async (content) => {
    ...
    try {
      ...
      // optimistic append from response
      if (res.data?.data) {
      ...
      // clear sending flag shortly after appending so auto-scroll won't be triggered by our own send
